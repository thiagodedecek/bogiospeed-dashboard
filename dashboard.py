...
# Carregar dados ignorando linhas vazias do Google Sheets
try:
    all_rows = sheet.get_all_records()
    # Filtra para manter apenas linhas que tenham o JOB Nº preenchido
    data = [r for r in all_rows if str(r.get('JOB Nº', '')).strip() != ""]
...
with st.expander("➕ ADD NEW INVOICE", expanded=True):
    with st.form(key="form_new_entry", clear_on_submit=True):
        ...
        if st.form_submit_button("SAVE DATA"):
            if f_job:
                ...
                # Lógica Inteligente para ignorar linhas "fantasmas" (image_165f85.png)
                # Conta apenas células que têm texto na coluna A
                content_col_a = [x for x in sheet.col_values(1) if x.strip() != ""]
                next_row_index = len(content_col_a) + 1
                
                sheet.insert_row(new_row, index=next_row_index)
                st.success(f"Invoice {f_job} Saved Successfully!")
                st.rerun()
            else:
                st.error("Please fill the JOB Nº field.")

# --- 5. HISTÓRICO & EDIÇÃO ---
st.markdown("---")
if not df.empty:
    st.subheader("Registered Invoices")
    # Histórico filtrado (sem as linhas vazias de image_165f85.png)
    ...
    with st.expander("📝 EDIT / UPDATE EXISTING JOB"):
        ...
        job_to_edit = st.selectbox("Search JOB Nº to Update", [""] + job_list)
        if job_to_edit:
            row_idx = df[df['JOB Nº'].astype(str) == job_to_edit].index[0]
            # O índice real na planilha precisa considerar o cabeçalho (+2)
            google_row_idx = row_idx + 2
            curr = df.iloc[row_idx]
            with st.form(key="form_edit_job"):
